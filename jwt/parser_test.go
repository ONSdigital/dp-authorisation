...
var (
	...
	unknownTokenKeyID             = "eyJraWQiOiJNZUtiNjUxOTRKbz0iLCJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..Oaea802wvGiXYzEjI8uZkg8LJoTAXlXDCImncRHBxyEpqer4Vj66dDxDj7z1Orcr_K5tbP8zY10BYs-q5aq6jb6NsaWyKRxIN5KmrLdFyGuWr-HdBL-JI1C2s4Ddvyrt-iEYlqI2axeshRbHFWWuz00Qt5Z7mw2RZg_vnzSN0WAw9imjKLbmDGIc8BmnwoEkgCl8eZ6ghwrirs7IdhlIocFXUdHi-WFaib9BzjpKdL0brDn0Czd0bBKek7ZbwXDN7RgwQz8kyKlgK2P-D_XdmNdio_5eqnKBv9ay5vt36dLYmSs0w7TPNfjhr5wA6wgJa9j9MaB3-_J6lvI4ccilJQ"
...
func TestCognitoRSAParser_Parse_UnkownTokenKeyID(t *testing.T) {
	p, _ := jwt.NewCognitoRSAParser(testPublicKey)

	Convey("Given a JWT token that has an unknown Key ID in header", t, func() {
		Convey("When Parse is called", func() {
			entityData, err := p.Parse(unknownTokenKeyID)

			Convey("Then the expected error is returned", func() {
				So(err, ShouldNotBeNil)
				So(err.Error(), ShouldEqual, jwt.ErrJWTKeySet.Error())
			})
