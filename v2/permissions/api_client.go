...
func (c *APIClient) GetPermissionsBundle(ctx context.Context) (Bundle, error) {
	...
	for retryCount, backOff := range c.backoffSchedule {
		...
		log.Info(ctx, "unexpected status returned from the permissions api permissions-bundle endpoint - retrying...", log.Data{"response": httpStatus}, log.Data{"retryInterval": backOff})
		time.Sleep(backOff)
	}
